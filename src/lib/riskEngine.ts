...
export function calculateRiskScore(answers: Record<string, any>, questions?: Question[]): RiskAnalysis {
    ...
    if (questions && questions.length > 0) {
        ...
        if (E > 50) recs.add(VAS_CATALOG.find(r => r.id === "armis_asset")!); // Exposure
        if (C < 60) recs.add(VAS_CATALOG.find(r => r.id === "zscaler_ztna")!); // General Controls
        if (B > 50) recs.add(VAS_CATALOG.find(r => r.id === "siemens_predictive")!); // Business Impact (Predictive)

        // Cyber Specific
        if (get("patching") < 100) recs.add(VAS_CATALOG.find(r => r.id === "agentless_patch")!);
        ...
        if (industry?.includes("Manufact") || industry?.includes("Logistics")) {
            recs.add(VAS_CATALOG.find(r => r.id === "era")!); // Electrical Risk
            recs.add(VAS_CATALOG.find(r => r.id === "cpm_fleet")!); // Fleet
        }
        if (industry?.includes("Infra") || industry?.includes("Power")) {
            recs.add(VAS_CATALOG.find(r => r.id === "drone_solar")!);
            recs.add(VAS_CATALOG.find(r => r.id === "fire_ball")!);
        }
        ...
        const hasManyIoT = iotCount > 20 || iotTypes.includes("PLCs");
        ...
        // --- FALLBACK RECOMMENDATIONS (Ensure not empty) ---
        if (recs.size === 0 && industry) {
            if (isOTHeavy) {
                // Manufacturing/Logistics default
                const def = VAS_CATALOG.find(r => r.id === "era"); // Electrical Risk Assessment (Safe bet)
                if (def) recs.add(def);
            } else {
                // Service/Retail default
                const def = VAS_CATALOG.find(r => r.id === "knowbe4_training"); // Phishing (Universal)
                if (def) recs.add(def);
            }
        }
