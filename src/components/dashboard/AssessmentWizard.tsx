...
import { speakText, startRecording, interpretVoice } from "@/lib/voice";
...
export const AssessmentWizard: React.FC<AssessmentWizardProps> = ({ onComplete, onBack }) => {
    ...
    const getFieldForStep = (currentStep: WizardStep): string | null => {
        switch (currentStep) {
            case "CUSTOMER_TYPE": return "customerType";
            case "VERIFICATION_METHOD": return "verificationMethod";
    ...
    const processVoiceInput = async (text: string | null, field: string, setter?: (val: string) => void, onComplete?: (text: string) => void) => {
        console.log(`[VoiceDebug] Processing input for ${field}: ${text}`);
        if (text) {
            // Interpret Intent (Hindi -> English)
            const interpretedText = await interpretVoice(text, field);
            console.log(`[VoiceDebug] Interpreted: "${text}" -> "${interpretedText}"`);

            const cleanText = interpretedText.toLowerCase();

            // Specific Logic for Selection Steps
            if (field === "customerType") {
                if (cleanText.includes("existing")) {
                    ...
                    handleVoice("I didn't catch that. Please say existing or new customer.");
                }
            } else if (field === "verificationMethod") {
                if (cleanText.includes("policy")) {
                    ...
                    handleVoice("Currently we support Policy Number or Mobile Number. Please say one of those.");
                }
            } else if (field === "industry") {
                if (cleanText.includes("retail") || cleanText.includes("commerce")) {
                    setIndustry("retail");
                    nextStep("COMPANY", "Please enter your company name.");
                } else if (cleanText.includes("manufactur")) {
                    setIndustry("manufacturing");
                    nextStep("COMPANY", "Please enter your company name.");
                } else if (cleanText.includes("health")) {
                    setIndustry("healthcare");
                    nextStep("COMPANY", "Please enter your company name.");
                } else if (cleanText.includes("tech")) {
                    setIndustry("technology");
                    nextStep("COMPANY", "Please enter your company name.");
                } else if (cleanText.includes("hospitality") || cleanText.includes("hotel")) {
                    setIndustry("hospitality");
                    nextStep("COMPANY", "Please enter your company name.");
                } else {
                    handleVoice("Please say an industry like Retail, Manufacturing, or Healthcare.");
                }
            } else if (field === "confirmation") {
                if (cleanText.includes("confirm") || cleanText.includes("yes") || cleanText.includes("proceed") || cleanText.includes("analyze") || cleanText.includes("correct")) {
                    ...
                    handleVoice("Please say confirm to proceed.");
                }
            } else if (setter) {
                // Standard Input Field Logic
                setter(interpretedText);
                if (onComplete) {
                    setTimeout(() => onComplete(interpretedText), 100);
                }
            }
        }
    ...
    const handleGeolocation = () => {
        if (!navigator.geolocation) {
            handleVoice("Geolocation is not supported by your browser.");
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                ...
                const formattedLocation = [city, state, postcode].filter(Boolean).join(", ");

                setLocation(formattedLocation);
                handleProcessing({ location: formattedLocation }); // Auto-advance if found
            } catch (error) {
                console.error("Geocoding failed:", error);
                setErrors(prev => ({ ...prev, location: "Could not fetch address details." }));
            } finally {
                setIsLocating(false);
            }
        }, (error) => {
    ...
    const startRecordingForField = async (field: string, setter?: (val: string) => void, onComplete?: (text: string) => void) => {
        setRecordingField(field);
        try {
            // Pass auto-stop callback
            const { stop } = await startRecording((text) => {
                console.log("[VoiceDebug] Auto-stop triggered via VAD");
                processVoiceInput(text, field, setter, onComplete);
    ...
    const toggleRecording = async (field: string, setter?: (val: string) => void, onComplete?: (text: string) => void) => {
        ...
        if (recordingField === field) {
            console.log("[VoiceDebug] Stopping recording manually...");
            // Stop Recording
            if (stopRecordingRef.current) {
                const text = await stopRecordingRef.current();
                console.log("[VoiceDebug] Manual Transcription received:", text);
                if (text) {
                    processVoiceInput(text, field, setter, onComplete);
                }
                stopRecordingRef.current = null;
            ...
            await startRecordingForField(field, setter, onComplete);
        }
    };
                            ...
                            <div className="space-y-2 relative">
                                <div className="relative">
                                    <button
                                        ...
                                        className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-orange-400 transition-colors z-10 p-1"
