...
export function OptimizationModal({ isOpen, onClose, analysis }: OptimizationModalProps) {
    ...
    useEffect(() => {
        ...
        // Cap reduction at 50% of original score for realism (same logic as engine)
        const actualReduction = Math.min(totalReduction, analysis.score * 0.6);
        setSimulatedScore(Math.max(0, Math.round(analysis.score - actualReduction)));
                                        ...
                                        >
                                            <div className="flex justify-between items-start relative z-10">
                                                <div className="flex gap-4">
                                                    {/* Checkbox UI */}
                                                    <div className={`mt-1 w-5 h-5 rounded border flex items-center justify-center transition-colors ${isSelected ? "bg-emerald-500 border-emerald-500" : "border-slate-500 group-hover:border-slate-300"
                                                        }`}>
