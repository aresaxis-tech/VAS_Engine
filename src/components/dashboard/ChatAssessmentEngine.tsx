...
import { speakText, stopSpeaking, startRecording, interpretVoice } from "@/lib/voice";
...
export const ChatAssessmentEngine: React.FC<ChatAssessmentEngineProps> = ({ onComplete, industry, intelData, onBack, initialQuestions }) => {
    ...
    // Update Risk on Answer Change or Initial Load
    useEffect(() => {
        if (activeQuestions.length > 0) {
            // Inject industry context for accurate scoring
            const effectiveAnswers = { industry_selection: industry, ...answers };
            const risk = calculateRiskScore(effectiveAnswers, activeQuestions);
            setCurrentRisk(risk);
        }
    }, [answers, activeQuestions, industry]);

    const processVoiceInput = async (text: string | null) => {
        console.log(`[ChatDebug] Processing Voice Input: ${text}`);
        if (text && text.trim().length > 0) {
            // Determine Context
            const currentQ = activeQuestions[currentIndex];
            const context = currentQ ? `Question: ${currentQ.text}` : "General Chat";

            // Interpret (Hindi -> English)
            const interpretedText = await interpretVoice(text, context);
            console.log(`[ChatDebug] Interpreted: "${text}" -> "${interpretedText}"`);

            setInputValue(interpretedText);
            handleAnswer(interpretedText);
            setInputValue("");
        }
    ...
    const startListening = async () => {
        ...
        try {
            const { stop } = await startRecording((text) => {
                console.log("[ChatDebug] Auto-Stop via VAD");
    ...
    const stopListening = async () => {
        ...
        setIsRecording(false);
        console.log("[ChatDebug] Stopping recording manually...");
        if (stopRecordingRef.current) {
            const text = await stopRecordingRef.current();
            console.log("[ChatDebug] Manual Transcription received:", text);
            stopRecordingRef.current = null;
            if (text) {
                processVoiceInput(text);
            }
    ...
    const handleAnswer = async (value: any, label?: string) => {
        ...
        if (!q) {
            console.warn("handleAnswer called but question is undefined. Index:", currentIndex);
            return;
        }
                    ...
                    <div className="max-w-xl mx-auto">
                        {!isTyping && currentQ && (
                            <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
                                ...
                                {/* Always Visible Speak Button (if not already handled inside text input) */}
                                {(currentQ.type === "yes_no" || currentQ.type === "select") && (
                                    <div className="flex justify-center pt-2">
                                        <Button
                                            type="button"
                                            onClick={toggleRecording}
                                            variant="secondary"
                                            className={`h-12 w-full sm:w-auto ${isRecording ? "bg-red-500/10 text-red-500 border-red-500/50 hover:bg-red-500/20" : "bg-slate-800/50 text-slate-400 hover:text-white"}`}
                                        >
