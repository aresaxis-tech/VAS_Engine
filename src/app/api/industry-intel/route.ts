...
// Note: This cache persists as long as the Next.js server sequence is running.
// It will reset on server restart. This is sufficient for the "session consistency" requirement.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `${industry.toLowerCase()}-${(subSector || "general").toLowerCase()}`;

    // 2. CHECK CACHE
    if (INTEL_CACHE.has(cacheKey)) {
      console.log(`⚡ CACHE HIT for: ${cacheKey}`);
      return NextResponse.json(INTEL_CACHE.get(cacheKey));
    }

    console.log(`🐢 CACHE MISS for: ${cacheKey}. Fetching AI...`);
    ...
    let responseText = await invokeClaude(systemPrompt, userMessage, 1024);
    ...
    // Check cache for fallback content too? 
    // Usually fallback is static so "caching" isn't strictly necessary, but helpful for uniformity.
    const cacheKey = `${industry.toLowerCase()}-fallback`;
    if (INTEL_CACHE.has(cacheKey)) {
      return NextResponse.json(INTEL_CACHE.get(cacheKey));
    }
    ...
    INTEL_CACHE.set(cacheKey, dbData);
